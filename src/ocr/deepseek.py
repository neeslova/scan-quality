...
RESOLUTION_MODES = {
    ...
    # Режимы, которых в карточке нет. Заводились, когда `base` и `gundam` не
    # влезали в T4, — но причина была не в разрешении, а в длине генерации:
    # с потолком `MAX_NEW_TOKENS` на карту помещаются все шесть режимов
    # (замерено на T4: пик 6.4 ГБ у `tiny`, 8.4 ГБ у `base` при потолке 14.56).
    #
    # Оставлены как предупреждение. `mid` на хорошей странице **сорвался в
    # зацикливание** — 5200 символов «an, an, on, on, to, to»: 768 не входит в
    # набор разрешений, на которых модель обучалась, и внештатный масштаб её
    # ломает. В прогон брать только режимы из карточки.
    "mid": (768, 768, False),
    "tiles": (768, 640, True),
}
...
#
# Замер на T4 подтвердил выбор с другой стороны — по качеству. На одной странице:
# `tiny` (64 визуальных токена) разваливает таблицу и путает слова, `base` (256)
# читает её верно. `gundam` читает не лучше `base`, но стоит 856 токенов против
# 256 — платить втрое не за что.
