...
class VerdictConfig(_Section):
    ...
    # Метки, которые считаются и попадают в отчёт, но не решают вердикт.
    #
    # Нужно для метки, не различающей классы на конкретном корпусе. Выбросить её
    # из `labels` нельзя — список дефектов зафиксирован ТЗ и общий для всех
    # корпусов, — а оставить в вердикте значит отдать решение шуму: при правиле
    # «хоть одна метка выше порога» одна вырожденная метка объявляет браком всё.
    # Измерять её мы не перестаём: в отчёте она видна, и видно, что она молчит.
    ignore: list[str] = Field(default_factory=list)
    # Обученная модель сведения (`src.models.from_metrics --save`). Пусто —
    # работает прежнее правило «один минус худший дефект».
    #
    # Правило задано руками и ничего не знает о том, какие метки на корпусе
    # информативны: достаточно одной вырожденной, чтобы утопить страницу. Модель
    # подбирает веса по эталону, и на `Data iz tg` это 0.770 против 0.505.
    # Путь, а не флаг: модель обучена на конкретном корпусе и на другой корпус
    # без переобучения не переносится — пусть это будет видно в конфиге.
